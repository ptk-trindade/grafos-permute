...
flowPerEdge {&edge, flow}: Flow per edge
*/
func fordFulkersonV3(adjacency [][]*Edge, source uint32, sink uint32) (float64, []EdgeFlow) {
	var maxFlow float64 = 0.0
	correlation := make([][2]*Edge, 0) // original edge, residual edge
	...
	for { // while there is a path from source to sink
		...
		var capacityChan = make(chan float64, 1)
		wg.Add(1)
		go func() {
			defer wg.Done()
			upPath := make([]*Edge, 0)
			// path in source direction
			upPath = append(upPath, upEdge)
			sourcePathCapacity := upPath[0].weight
			for upPath[len(upPath)-1].origin != source {
				upPath = append(upPath, father[upPath[len(upPath)-1].origin].edge)
				if upPath[len(upPath)-1].weight < sourcePathCapacity {
					sourcePathCapacity = upPath[len(upPath)-1].weight
				}
			}

			capacityChan <- sourcePathCapacity
			pathCapacity := <-capacityChan

			for _, edge := range upPath {
				edge.weight -= pathCapacity
				edge.comp.weight += pathCapacity
			}

		}()

		// path in sink direction
		downPath := make([]*Edge, 0)
		downPath = append(downPath, downEdge)
		sinkPathCapacity := downEdge.weight
		for downPath[len(downPath)-1].dest != sink {
			downPath = append(downPath, father[downPath[len(downPath)-1].dest].edge)
			if downPath[len(downPath)-1].weight < sinkPathCapacity {
				sinkPathCapacity = downPath[len(downPath)-1].weight
			}
		}

		sourcePathCapacity := <-capacityChan
		...
		capacityChan <- pathCapacity

		maxFlow += pathCapacity

		// update residual graph
		for _, edge := range downPath {
			edge.weight -= pathCapacity
			edge.comp.weight += pathCapacity
		}
